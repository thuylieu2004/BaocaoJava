...
import { createsach, getSachById, updateSach } from "../../service/Sachservice";
...
const UpdateSach = () => {
    ...
    useEffect(() => {
        ...
        getSachById(sachId).then((Response) => {
            const data = Response.data;
            setTenSach(data.tenSach);
            setMaTacGia(data.maTacGia);
            setMaNhaXuatBan(data.maNhaXuatBan);
            setmaTheLoai(data.maTheLoai);
            setGiaBan(data.giaBan);
            setSoLuong(data.soLuong);
            setMoTa(data.moTa);
            setHinhAnh(data.hinhAnh);
        }).catch((error) => {
            console.error("Lỗi không tìm thấy sách:", error);
        });
    ...
    return (
        <div className="container">
            <div className="row">
                <div className="card">
                    <h2 className="text-center">Sửa thông tin sách</h2>
                    <div className="card-body">
                        <form>
                            ...
                            <button className="btn btn-success" onClick={UpdateSach}>Sửa sách</button>
                        </form>
                    </div>
                </div>
            </div>
